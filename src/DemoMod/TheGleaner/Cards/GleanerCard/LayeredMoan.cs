...
public class LayeredMoan : CustomCardModel {
    ...
    protected override IEnumerable<IHoverTip> ExtraHoverTips => [HoverTipFactory.FromKeyword(CardKeyword.Exhaust), HoverTipFactory.FromKeyword(CustomEnums.Dissonance)];
    ...
    public override async Task AfterCardEnteredCombat(CardModel card) {
        ...
        foreach (CombatHistoryEntry entry in CombatManager.Instance.History.Entries.Where(e => e is CardExhaustedEntry or CardGeneratedEntry)) {
            switch (entry) {
                ...
                case CardGeneratedEntry cardGeneratedEntry when RandomDissonanceCard.transformedDissonanceCards().Any(c => c.Id.Equals(cardGeneratedEntry.Card.Id)):
                    count++;
                    break;
            }
        }
