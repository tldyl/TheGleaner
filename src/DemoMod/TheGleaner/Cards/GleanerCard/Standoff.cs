...
public class Standoff : CustomCardModel {
	...
	protected override IEnumerable<DynamicVar> CanonicalVars =>
		new List<DynamicVar> { new DamageVar(19, ValueProp.Move) };

	public Standoff() : base(2, CardType.Attack, CardRarity.Uncommon, TargetType.AllEnemies, true, true) {
	}
	...
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay) {
		...
		foreach (Creature creature in allEnemies) {
			GleanerVfxCmd.PlayOnCreature<Node2D>(creature, "res://TheGleaner/scenes/vfx/arrow_attack.tscn", 0.3f);
		}
