...
public class Standoff : CustomCardModel {
    ...
    public Standoff() : base(0, CardType.Attack, CardRarity.Common, TargetType.AllEnemies) {
        
    }

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay) {
        IEnumerable<Creature> allEnemies = CombatState.HittableEnemies.Where(enemy => enemy.Monster.IntendsToAttack);
        IEnumerable<DamageResult> _ = await CreatureCmd.Damage(choiceContext, allEnemies, DynamicVars.Damage, Owner.Creature,
            this);
    }
