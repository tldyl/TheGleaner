...
public class OneCostAttacks : CustomCardModel, IChoosable
{
    ...
    public OneCostAttacks() : base(-1, CardType.Attack, CardRarity.Common, TargetType.None)
    {
    }
    ...
    public async Task OnChosen(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        IEnumerable<CardModel> cards =
            from c in list1.SelectMany(c =>
                c.GetUnlockedCards(
                    Owner.UnlockState,
                    Owner.RunState.CardMultiplayerConstraint
                ))
            where c.Type == CardType.Attack
                  && c.EnergyCost.Canonical == 1
            select c;

        List<CardModel> list2 = CardFactory.GetDistinctForCombat(
            Owner,
            cards,
            _dynamicVars[0].IntValue,
    ...
    public void addVar(DynamicVar dynamicVar)
    {
        ...
        Dictionary<string, DynamicVar> vars =
            (Dictionary<string, DynamicVar>)AccessTools
                .Field(typeof(DynamicVarSet), "_vars")
                .GetValue(DynamicVars);

        vars.TryAdd(dynamicVar.Name, dynamicVar);
    }
}